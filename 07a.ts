...
function getChildren(str: string) {
  ...
  return parts.map((part) => {
    const matches = part.match(/(\d+) (.*) bag/)!;
...
type Bag = { title: string; children: ReturnType<typeof getChildren> };
type BagTree = Record<string, undefined | Bag>;
const bags = input
  .trim()
  .split('\n')
  .map((str) => {
    ...
    const title = parts[0].match(/(.*) bag/)![1];
    const children = getChildren(parts[1]);
...
function canBagContainTarget(title: string, t: string) {
  // console.log('Can bag', title, 'contain', t, '?');
  const bag = bags[title];
  if (!bag) {
    return false;
  }
  for (const { title } of bag.children) {
    if (title === t) {
      return true;
    }
    if (canBagContainTarget(title, t)) {
      return true;
    }
  }
  return false;
...
const allBags = Object.values(bags).filter((bag) =>
  canBagContainTarget(bag!.title, target),
);
console.log(
  'These bags can contain',
  target,
  ':\n',
  allBags.map((b) => `  - ${b!.title}`).join('\n'),
);
console.log(allBags.length);

// console.log(JSON.stringify(bags, null, 4));
