...
function getAdjacentSeats(
  ...
  const adjacentSeatRules: RuleTuple[] = [
    [-1, -1],
    [-1, 0],
    [-1, 1],
    [0, -1],
  ...
  const seats: Type[] = [];
  for (const rule of adjacentSeatRules) {
    const seat = findAdjacentSeat(plan, rowIndex, columnIndex, rule);
    if (seat) {
      seats.push(seat);
    }
  }
  ...
  const newPlan = plan.reduce((newPlan, row, rowIndex) => {
    newPlan.push(
      row.map((seat, columnIndex) => {
        const adjacentSeats = getAdjacentSeats(plan, rowIndex, columnIndex);
