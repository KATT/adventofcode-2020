...
const parsed = input
  .trim()
  .split('\n')
  .map((str) => Array.from(str).map((char) => char as Type));

function getAdjacentSeats(plan: Type[][], rowIndex: number, seatIndex: number) {
  const adjacentSeatCoords = [
    [-1, -1],
    [-1, 0],
    [-1, 1],
    [0, -1],
  ...
  const seats: Type[] = [];
  for (const [rowDiff, seatDiff] of adjacentSeatCoords) {
    const seat = plan[rowIndex + rowDiff]?.[seatIndex + seatDiff];
    if (seat) {
      seats.push(seat);
    }
  }
...
function exec(plan: Type[][]) {
  const newPlan = plan.reduce((newPlan, row, rowIndex) => {
    newPlan.push(
      row.map((seat, seatIndex) => {
        const adjacentSeats = getAdjacentSeats(plan, rowIndex, seatIndex);
        if (seat === Type.EMPTY_SEAT) {
          const numOccupied = adjacentSeats.reduce(
            (sum, seat) => (seat === Type.OCCUPIED_SEAT ? sum + 1 : sum),
            0,
          );
          if (numOccupied === 0) {
            return Type.OCCUPIED_SEAT;
          }
        } else if (seat === Type.OCCUPIED_SEAT) {
          const numOccupied = adjacentSeats.reduce(
            (sum, seat) => (seat === Type.OCCUPIED_SEAT ? sum + 1 : sum),
            0,
          );
          if (numOccupied >= 4) {
            return Type.EMPTY_SEAT;
          }
        }
        return seat;
      }),
...
console.log(
  ...
  next.reduce(
    (sum1, row) =>
      sum1 +
      row.reduce(
        (sum2, seat) => sum2 + (seat === Type.OCCUPIED_SEAT ? 1 : 0),
        0,
      ),
    0,
  ),
